...
import { ref as k, shallowRef as Y, defineComponent as ee, inject as te, computed as N, onMounted as ge, onUnmounted as fe, openBlock as O, createElementBlock as D, createVNode as m, unref as c, createElementVNode as G, createBlock as Q, createCommentVNode as j, Fragment as Me, renderList as he, normalizeClass as we, withCtx as M, createTextVNode as L, toDisplayString as A, mergeProps as Se, toRaw as Ve, reactive as ye } from "../../../assets/vue.js";
import { VcsEvent as ae, CesiumMap as P, PanoramaMap as w, ObliqueMap as V, OpenlayersMap as R, AbstractInteraction as be, EventType as Ce, panoramaFeature as Z, getHeightFromTerrainProvider as Ne, Viewpoint as z, EventHandler as Ee, PanoramaFeatureHighlight as _e, PanoramaDatasetLayer as Ie } from "../../../assets/core.js";
import { VContainer as Oe, VRow as B, VCol as C } from "../../../assets/vuetify.js";
const h = "@vcmap/multi-view", Pe = "3.0.2", qe = "^6.2";
class ke extends ae {
  constructor() {
    super(...arguments);
...
function Ae(e, i) {
  ...
  async function ie(o) {
    ...
    if (o === R.className)
      ...
      const d = _();
      d && (r = new E(), await r.setCollection(d));
    }
  ...
  async function se() {
    ...
    i.allowedSideMaps && i.allowedSideMaps.length > 0 && (a.value = [...i.allowedSideMaps]), e.obliqueCollections.size === 0 && (a.value = a.value.filter(
      (r) => ![V.className, E.className].includes(
        r
      )
    )), [...e.layers].some((r) => r instanceof Ie) || (a.value = a.value.filter(
      (r) => w.className !== r
    )), s.value = ((o = e.maps.activeMap) == null ? void 0 : o.className) ?? null, a.value.length > 0 && (i.startingSideMap && a.value.includes(i.startingSideMap) ? await I(i.startingSideMap) : a.value.includes(E.className) ? await I(E.className) : await I(a.value[0]));
  }
  const oe = [
...
const $e = { class: "multi-view-container" }, Te = { class: "d-flex justify-start pa-1 ga-1" }, Fe = {
  ...
  setup(e) {
    ...
    function l(n) {
      return n === E.className ? "multiView.obliqueMultiView" : `navbar.maps.${n}`;
    }
    const s = N(() => {
      var n;
    ...
    }), v = N(() => a.availableSideMapClasses.value.map((n) => ({
      title: l(n),
      value: n
    })));
    function y(n) {
    ...
    return (n, g) => (O(), D("div", $e, [
      ...
      u.value === c(E).className ? (O(), D("div", Fe, [
        (O(), D(
          Me,
          null,
          he(p, (f) => G(
...
const xe = /* @__PURE__ */ ee({
  ...
  setup(e) {
    const i = e, t = x(), a = i.getConfig(), l = k({
      ...t,
      ...structuredClone(a)
    }), s = t.allowedSideMaps, u = N(() => l.value.allowedSideMaps && l.value.allowedSideMaps.length > 0 ? l.value.allowedSideMaps : s), y = [...te("vcsApp").obliqueCollections].map(
      (p) => {
        var n;
        return {
          value: p.name,
